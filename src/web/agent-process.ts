...
export function startAgentProcess(name: string): { ok: boolean; pid?: number; error?: string } {
  ...
  try {
    ...
    const cmd = `export PATH="/opt/homebrew/bin:$HOME/.bun/bin:/usr/local/bin:/usr/bin:/bin:$PATH" && unset TELEGRAM_BOT_TOKEN && export TELEGRAM_STATE_DIR="${tgStateDir}" && ${ollamaEnv}cd "${dir}" && ${CLAUDE} --continue ${skipFlag}--model ${model} --channels plugin:telegram@claude-plugins-official`
    execSync(
